...
Three falsifiable drills that push the chapter's CPU+I/O lesson past the book, all built on the
...
Together they tell one story: **async hides I/O behind I/O, but a heavy GIL-bound CPU stage
becomes a floor that only multiple processes can lower — and the right structure overlaps CPU
with I/O, which naive per-page multiprocessing throws away.** h01 finds the floor; h02 breaks
through it with a process pool; h03 shows that the async+pool *hybrid* even beats plain
multiprocessing, because it keeps the CPU/I/O overlap that per-page processes lose.
...
| h03 | [multiprocessing vs hybrid](h03_multiprocessing_vs_hybrid/) | **OVERTURNED** | predicted a tie; the async+pool hybrid is ~1.23× *faster* than per-page multiprocessing at matched parallelism, because MP serializes each page's OCR and CPU while the hybrid overlaps them |
...
*why*, *what to do about it*, and *whether a simpler tool would do better*:
...
- **h03** asks the obvious follow-up — since pages are independent, why not skip asyncio and run
  each page in its own process? It turns out the hybrid still *wins*, because per-page
  multiprocessing serializes each page's OCR and CPU inside one worker, discarding the very
  overlap the chapter is about. A surprise that overturned the predicted tie.
